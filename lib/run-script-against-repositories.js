...
import { red, gray } from "fmt/colors.ts";
...
export async function runScriptAgainstRepositories(state, octoherdRepos = []) {
  if (!state.octoherdReposPassedAsFlag) {
    ...
    const prompt = new enquirer.List({
      ...
      separator: / +/,
      hint: "e.g. octoherd/cli. Use a * to load all repositories for an owner, e.g. octoherd/*. Enter nothing to exit",
      validate(input) {
        ...
        return (
          red(`"${invalid}" is not a valid repository name.`) +
          gray(" The format is <owner>/<repo>")
        );
      },
    });
