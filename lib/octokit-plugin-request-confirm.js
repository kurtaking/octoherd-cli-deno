...
export function requestConfirm(octokit, { octoherd }) {
  ...
  octokit.hook.wrap("request", async (request, options) => {
    ...
    const { method, url, baseUrl, ...parameters } = options;
    ...
    const parameterPadding = Math.max(
      5,
      ...Object.keys(parameters).map((p) => p.length),
    ) + 1;
    ...
    const prompt = new enquirer.Select({
      name: "allow",
      message: `Do you want to allow the following request:
...
${
        Object.entries(parameters)
          .map(([key, value]) =>
            `  ${(key + ":").padEnd(parameterPadding)} ${value}`
          )
          .join("\n")
      }
